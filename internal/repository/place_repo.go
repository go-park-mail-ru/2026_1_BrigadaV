...
func (r *PlaceRepo) Search(ctx context.Context, query string) ([]models.Place, error) {
	searchQuery := `
		...
		LEFT JOIN country c ON l.country_id = c.id
		LEFT JOIN category cat ON p.category_id = cat.id
		WHERE to_tsvector('russian', p.name || ' ' || COALESCE(p.description, '')) @@ plainto_tsquery('russian', $1)
		ORDER BY ts_rank(to_tsvector('russian', p.name || ' ' || COALESCE(p.description, '')), plainto_tsquery('russian', $1)) DESC`
