...
func (r *UserRepo) Create(ctx context.Context, user *models.User) error {
	...
	err := r.db.QueryRow(ctx, query, user.Login, user.Nickname, user.AvatarURL,
		user.PasswordHash, user.Country, user.City, user.About).
		Scan(&user.ID, &user.CreatedAt, &user.UpdatedAt)
	return err
}

func (r *UserRepo) GetByLogin(ctx context.Context, login string) (*models.User, error) {
	query := `SELECT id, login, nickname, avatar_url, password_hash, country, city,
	          about, has_reviews, created_at, updated_at
...
func (r *UserRepo) Update(ctx context.Context, user *models.User) error {
	query := `UPDATE "user" SET login = $1, nickname = $2, avatar_url = $3,
	          country = $4, city = $5, about = $6, has_reviews = $7, updated_at = NOW()
	          WHERE id = $8 RETURNING updated_at`
	err := r.db.QueryRow(ctx, query, user.Login, user.Nickname, user.AvatarURL,
		user.Country, user.City, user.About, user.HasReviews, user.ID).
		Scan(&user.UpdatedAt)
	return err
}
